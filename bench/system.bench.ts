...
test('asyncComputed', async ({ bench }) => {
    await bench.compare(
...
test('effect stress', async ({ bench }) => {
    await bench('create + dispose 1000 effects (pool recycling)', () => {
...
test('deep propagation', async ({ bench }) => {
    await bench.compare(
...
test('stabilization', async ({ bench }) => {
    await bench.compare(
...
test('root', async ({ bench }) => {
    await bench('root scope creation + disposal', () => {
        root((dispose) => {
            dispose();
        });
    }).run();
